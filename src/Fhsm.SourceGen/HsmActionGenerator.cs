...
namespace Fhsm.SourceGen
{
    ...
    public class HsmActionGenerator : IIncrementalGenerator
    {
        ...
        private static void Execute(SourceProductionContext context, Compilation compilation, IEnumerable<MethodInfo> methods)
        {
            ...
            string namespaceName = assemblyName == "Fhsm.Kernel" ? "Fhsm.Kernel" : $"{assemblyName}.Generated";

            // Generate dispatch table
            var source = GenerateDispatchTable(actions, guards, namespaceName);
            context.AddSource("HsmActionDispatcher.g.cs", source);
        }

        private static string GenerateDispatchTable(List<MethodInfo> actions, List<MethodInfo> guards, string namespaceName)
        {
            ...
            // Note: Using IntPtr to store function pointers to avoid delegate allocation
            // The dictionary stores the raw address.
            ...
            // Registration for cross-assembly usage (Validation/Testing)
            ...
            // Retrieval for manual linking
            sb.AppendLine("        public static IntPtr GetAction(ushort id) => ActionTable.TryGetValue(id, out var ptr) ? ptr : IntPtr.Zero;");
            sb.AppendLine("        public static IntPtr GetGuard(ushort id) => GuardTable.TryGetValue(id, out var ptr) ? ptr : IntPtr.Zero;");
